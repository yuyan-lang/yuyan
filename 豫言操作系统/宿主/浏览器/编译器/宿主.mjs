...
export async function 执行模块(模块, 桥模块, 文件, 参数 = [], { 编译 = false, 组装, 报告 = () => {}, 原语扩展 = {}, 异步原语 = false, 时限 } = {}) {
  const 桥 = 创建值桥(桥模块);
  ...
  const 原语 = {
    ...
    豫言_格式化当前本地日期时间: 格式 => {const d=new Date(),pad=n=>String(n).padStart(2,'0'),fields={Y:String(d.getFullYear()),m:pad(d.getMonth()+1),d:pad(d.getDate()),H:pad(d.getHours()),M:pad(d.getMinutes()),S:pad(d.getSeconds()),'%':'%'};return 文(格式).replace(/%([%A-Za-z])/g,(_,k)=>{if(!(k in fields))throw Error('暂不支持日期格式：%'+k);return fields[k];});},
  ...
  if (编译) {
    ...
    原语.豫言_同步运行子进程 = (名, 参) => {
      ...
      const 汇编文 = 文(文件.读(项[1]));
      // 文言：汇编之界记于日志，以辨编器栈与组装器栈。汉语：标记 WAT 组装阶段，便于定位浏览器特有的栈错误。
      报告({ type: "stage", phase: "compile", label: "正在组装 Wasm（" + 汇编文.length + " 字符）" });
      文件.写(项[2], 组装(汇编文)); return true;
    };
