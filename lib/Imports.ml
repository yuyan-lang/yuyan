...
let get_module_real_path (module_name_parsed : A.t) : string proc_state_m (* path *) =
  ...
  | A.N (N.StructureDeref _, _) | A.FreeVar _ ->
    ...
    if (not (Filename.is_relative file_name)) && Sys.file_exists file_name
    then return file_name
    else if Sys.file_exists (Filename.concat cur_module_dir head_name)
    then return (Filename.concat cur_module_dir file_name)
