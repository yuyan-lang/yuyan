...
test("签名目录覆盖标准库所有模块且分页不丢失声明", async () => {
  const 根 = new URL("../../../藏书阁/", import.meta.url);
  const 遍历 = async 相对 => {
    ...
    return (await Promise.all(项们.filter(项 => !项.name.endsWith("_v0")).map(async 项 => {
      const 路径 = 相对 + 项.name;
      return 项.isDirectory() ? 遍历(路径 + "/") : 路径.endsWith("。豫") && !路径.endsWith("。测试。豫") ? [路径] : [];
    }))).flat();
  };
  const 目录 = JSON.parse(await readFile(new URL("../知识/标准库签名.json", import.meta.url), "utf8"));
  assert.deepEqual(目录.模块.map(项 => 项.路径).sort(), ["标准库。豫", ...await 遍历("标准库/")].sort());
  const 知识 = 构建知识("指南", 目录, {});
