...
int main(int argc, char **argv) {
 ...
 for(int 轮=0; 轮<=5; ++轮) {
  ...
  else if(!strcmp(argv[1],"树基准")) { 树 *根=建树(数); 果=树和(根); 释树(根); }
  else if(!strcmp(argv[1],"数组基准")) { /* 文言：有界复用，以同策较之。汉语：与私有缓冲原型对照，最多保留 128 MiB，超限仍逐轮分配释放。 */
   static int64_t *缓存; static int64_t 容量;
   int 复用 = 数 >= 0 && 数 <= 16777215;
   if(复用 && 数 > 容量) { int64_t *新缓存=realloc(缓存,数*sizeof(*缓存)); if(!新缓存) return 1; 缓存=新缓存; 容量=数; }
   int64_t *组=复用?缓存:malloc(数*sizeof(*组)); if(!组 && 数) return 1;
   for(int64_t i=数;i;i--)组[i-1]=i;
   for(int64_t i=数;i;i--)果+=组[i-1]; if(!复用) free(组);
  } else return 2;
  double 秒=单调秒()-始;
