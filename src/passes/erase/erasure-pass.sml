...
(* ARCHAIC, we should perhaps remove this file *)
type Expr = RExpr
type Signature = RSignature
  ...
  fun kaddToCtxR (m : kmapping ) (c : kcontext)= appendRelativeMappingToCurrentContext m c(* R for relative *)
    ...
    and eraseSigLazy (kont : (kcontext -> PersistentKMachine.pkcomputation) option) (ctx : kcontext) (s : RSignature) : PersistentKMachine.pkcomputation =
        ...
        (
         ...
         | RTypeMacro (n, t)::ss => eraseSigLazy kont (kaddToCtxR (TypeDef([n],t, ())) ctx) ss
        | RTermTypeJudgment(n, t):: ss => ( (* todo check all type decls are realized, otherwise 
    ...
    and synthesizeType x y = #2 (TypeCheckingPass.synthesizeType x y )
    ...
    (
        ...
        (* val _ = print ("eraseSynExpr on " ^ PrettyPrint.show_typecheckingExpr e ^ " in context " ^ PrettyPrint.show_typecheckingpassctx (eraseCtx ctx) ^ "\n"); *)
         val res = case e of
            RExprVar v => PKRet(klookup ctx v)
            | RUnitExpr => PKRet(PKUnit)
            | RProj(e, l) => (case synthesizeType (n ctx) e of
                    Prod ls => PKProj(eraseSynExpr ctx e,
                        klookupLabel ls l)
            )
            | RCase(e,cases) => (case (synthesizeType (n ctx) e) of
            ...
            | RLamWithType (t, ev, e) => PKRet(kabs(fn v => eraseSynExpr (kaddToCtxR (TermTypeJ([ev], t, v)) ctx) e))
            | RApp (e1, e2) => (case synthesizeType (n ctx) e1
                of Func (t1, t2) => 
                PKApp(eraseSynExpr ctx e1, eraseCkExpr ctx e2 t1))
            | RTAbs (tv, e2) => eraseSynExpr ctx e2
            | RTApp (e2, t) => eraseSynExpr ctx e2
            | ROpen (e1, (tv, ev, e2)) => (case synthesizeType (n ctx) e1 of
            ...
            | RUnfold e2 => PKUnfold(eraseSynExpr ctx e2) 
            | RStringLiteral s => PKRet(PKBuiltinValue(KbvString s))
    ...
    and eraseCkExpr  (ctx : kcontext) (e : Expr) (tt: Type) (* tt target type *) : pkcomputation =
        ...
        val res = case e of
            ...
            | RProj(e, l) => eraseSynExpr ctx (RProj(e,l))
            | RInj (l, e) => (case tt of
            ...
            | RFix (ev, e)=>  let val boundID = UID.next() in
    ...
    and lookAheadForValue (s : Signature) (ename : UTF8String.t) : (UTF8String.t * Expr) * Signature = 
        case s of
         RTermDefinition(n, e) :: ss => if UTF8String.semanticEqual n  ename then ((n, e), ss)
                                       else 
                                       let val ((n', e'), ss') = lookAheadForValue ss ename 
                                       in ((n', e'), RTermDefinition(n, e)::ss') end
