structure TypeCheckingErrors =
 struct 
    ...
    fun enrich (res : 'a witherrsoption) (diagnosticMessageNoNewline : string) : 'a witherrsoption = 
        let fun enrichErr e = 
            case e of 
                StaticError (loc, severity, msg, addmsg, related) =>
                let val newAddMsg = 
                    case addmsg of 
                        NONE => SOME diagnosticMessageNoNewline
                        | SOME amsg => SOME(diagnosticMessageNoNewline ^ "\n" ^ amsg)
                in 
                    StaticError (loc, severity, msg, newAddMsg, related) 
                end
        in
        case res of 
            Success a => Success a
            | DErrors l => DErrors (map enrichErr l)
            | NotAvailable => NotAvailable
        end
    fun enrichErrWhenChecking (e : RExpr ) (tt : CExpr) (res : 'a witherrsoption) = 
        enrich res ("在检查"^
          "`" ^ PrettyPrint.show_typecheckingRExpr e ^ "`拥有类型" ^
        "`" ^ PrettyPrint.show_typecheckingCType tt ^ "`时" 
        )
    fun enrichErrWhenSynthesizing (e : RExpr )  (res : 'a witherrsoption) = 
        enrich res ("在推断"^
          "`" ^ PrettyPrint.show_typecheckingRExpr e ^ "`的类型时"
        )
