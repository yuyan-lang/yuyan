...
元素('账户表单').addEventListener('submit',e=>{e.preventDefault();执行账户(async()=>{await 请求账户(注册模式?'register':'login',{email:元素('账户邮箱').value,password:元素('账户密码').value,acceptTerms:元素('接受条款').checked,termsVersion:'2026-09-06'});元素('账户密码').value='';await 刷账户();状态(注册模式?'账户已创建，可以生成上传令牌。':'已登录。',注册模式?'户已立，可生上传之符。':'已登入。');});});
