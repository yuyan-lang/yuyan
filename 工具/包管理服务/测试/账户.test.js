...
export function 数据库(){const sql=new DatabaseSync(':memory:');sql.exec('PRAGMA foreign_keys=ON');sql.exec(readFileSync(new URL('../迁移/0001_建立包管理.sql',import.meta.url),'utf8'));sql.exec(`INSERT INTO "包"("名称","所有者编号") VALUES ('旧包',1); INSERT INTO "上传" VALUES ('旧包','1','旧包-1.zip','abc',1,'pending/abc',1,CURRENT_TIMESTAMP); INSERT INTO "上传" VALUES ('待发布旧包','1','待发布旧包-1.zip','def',1,'pending/def',1,CURRENT_TIMESTAMP);`);sql.exec('BEGIN;'+readFileSync(new URL('../迁移/0002_邮箱注册.sql',import.meta.url),'utf8')+'COMMIT;');sql.exec(readFileSync(new URL('../迁移/0003_发布者验证.sql',import.meta.url),'utf8'));sql.exec(readFileSync(new URL('../迁移/0004_邮箱验证与密码重置.sql',import.meta.url),'utf8'));for(const file of ['0005_即时发布.sql','0006_所有者绑定.sql'])sql.exec(readFileSync(new URL('../迁移/'+file,import.meta.url),'utf8'));const db={邮件:[],prepare(query){return{args:[],bind(...args){this.args=args;return this;},first(){return sql.prepare(query).get(...this.args)??null;},run(){return sql.prepare(query).run(...this.args);}}},async batch(stmts){sql.exec('BEGIN');try{const out=stmts.map(s=>s.run());sql.exec('COMMIT');return out;}catch(e){sql.exec('ROLLBACK');throw e;}}};return {sql,db};}
const 基址='https://packages.example',密码='一段足够长而且不容易猜到的测试密码！';
export async function 请求(db,action,data,cookie='',ip='192.0.2.1',origin=基址){const headers={'CF-Connecting-IP':ip};if(cookie)headers.Cookie=cookie;if(data!==undefined){headers.Origin=origin;headers['Content-Type']='application/json';}return 账户入口(new Request(基址+'/api/account/'+action,{method:data===undefined?'GET':'POST',headers,body:data===undefined?undefined:JSON.stringify(data)}),{DB:db,EMAIL:{async send(message){db.邮件.push(message);}}});}
export const 注册=(db,email,ip)=>请求(db,'register',{email,password:密码,acceptTerms:true,termsVersion:条款版本},'',ip);
...
test('上传 Access Token 不能充当登录会话或操作账户',async()=>{
 ...
 sql.exec('UPDATE 邮箱账户 SET 邮箱已验证=1');const cookie=register.headers.get('set-cookie');const tokenReply=await 请求(db,'token',{},cookie);const {token,scope}=await tokenReply.json();assert.equal(scope,'packages:upload');
...
test('重置密码撤销全部旧会话、上传令牌和邮件凭据，重放及旧密码无效',async()=>{
 const {sql,db}=数据库();const r=await 注册(db,'reset@example.com');const cookie=r.headers.get('set-cookie');await 请求(db,'verify-email',{token:邮件券(db,'verify')},cookie);await 请求(db,'token',{},cookie);
 const unknown=await 请求(db,'forgot-password',{email:'missing@example.com'});const known=await 请求(db,'forgot-password',{email:'reset@example.com'});assert.equal(unknown.status,known.status);assert.deepEqual(await unknown.json(),await known.json());
 const token=邮件券(db,'reset'),password='全新密码-长到足够安全-123456789';
 assert.equal((await 请求(db,'reset-password',{token,password})).status,200);
 assert.equal(sql.prepare('SELECT COUNT(*) AS n FROM 登录会话').get().n,0);assert.equal(sql.prepare('SELECT COUNT(*) AS n FROM 访问令牌 WHERE 已启用=1').get().n,0);assert.equal(sql.prepare('SELECT COUNT(*) AS n FROM 邮件凭据').get().n,0);
 assert.equal((await 请求(db,'reset-password',{token,password})).status,400);assert.equal((await 请求(db,'login',{email:'reset@example.com',password:密码})).status,401);assert.equal((await 请求(db,'login',{email:'reset@example.com',password})).status,200);sql.close();
});
test('过期凭据、重发限流和发送失败不会验证账户',async()=>{
 const {sql,db}=数据库();const r=await 注册(db,'expiry@example.com');const cookie=r.headers.get('set-cookie'),token=邮件券(db,'verify');sql.exec('UPDATE 邮件凭据 SET 到期=0');assert.equal((await 请求(db,'verify-email',{token},cookie)).status,400);
 assert.equal((await 请求(db,'resend-verification',{},cookie)).status,200);assert.equal((await 请求(db,'resend-verification',{},cookie)).status,200);assert.equal((await 请求(db,'resend-verification',{},cookie)).status,429);
 sql.exec('DELETE FROM 账户限流');const req=new Request(基址+'/api/account/resend-verification',{method:'POST',headers:{Origin:基址,'Content-Type':'application/json','CF-Connecting-IP':'192.0.2.9',Cookie:cookie},body:'{}'});const before=sql.prepare('SELECT COUNT(*) AS n FROM 邮件凭据').get().n;
 assert.equal((await 账户入口(req,{DB:db,EMAIL:{send(){throw Error('模拟投递失败');}}})).status,503);assert.equal(sql.prepare('SELECT COUNT(*) AS n FROM 邮件凭据').get().n,before);assert.equal(sql.prepare('SELECT 邮箱已验证 AS v FROM 邮箱账户').get().v,0);sql.close();
});
