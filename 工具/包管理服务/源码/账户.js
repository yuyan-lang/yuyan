...
export async function 查会话(req,db,now){const token=会话值(req);if(!token)return null;return db.prepare('SELECT u."编号" AS id,u."名称" AS name,CASE WHEN u."已验证"=1 THEN \'verified\' ELSE \'unverified\' END AS verification,a."邮箱" AS email,a."邮箱已验证" AS emailVerified FROM "登录会话" s JOIN "用户" u ON u."编号"=s."用户编号" JOIN "邮箱账户" a ON a."用户编号"=u."编号" WHERE s."摘要"=? AND s."到期">?').bind(await 摘要(token),now).first();}
