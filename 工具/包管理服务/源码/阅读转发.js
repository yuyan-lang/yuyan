...
export async function 阅读页面入口(请求, 环境) {
  ...
  const 语言 = 网址.searchParams.get('lang') === 'wen' ? 'wen' : 'han';
  const 模式 = 网址.searchParams.get('view') === 'source' ? 'source' : 'docs';
  const 路径 = 网址.searchParams.get('path') || '';
  const 历史偏移 = Number(网址.searchParams.get('historyOffset') || 0);
  ...
  if (已存) return 请求.method === 'HEAD' ? new Response(null, 已存) : 已存;
  try {
    ...
    if (页签 === 'overview') {
      ...
      数据.path = 文件们.find(项 => 项.path === 路径)?.path || 文件们.find(项 => 项.path === 'source/总集。豫')?.path || 文件们[0]?.path || '';
      if (数据.path) 数据.file = await 公开数据(环境, 网址.origin, '/api/releases/' + 编号 + '/file?' + new URLSearchParams({ path: 数据.path, view: 模式 }));
    }
    ...
    const 页面 = new Response(回应.body, { headers: 标头 });
    if (缓存) { try { await 缓存.put(缓存键, 页面.clone()); } catch { /* 成页仍交读者。 */ } }
    return 请求.method === 'HEAD' ? new Response(null, { headers: 标头 }) : 页面;
  } catch (错误) {
