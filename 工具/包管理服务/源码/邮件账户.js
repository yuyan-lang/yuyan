...
export async function 使用邮件凭据(db,purpose,token,now,userId,passwordData){
  ...
  const where='SELECT "用户编号" FROM "邮件凭据" WHERE "摘要"=? AND "用途"=? AND "到期">?'+(purpose==='verify'?' AND "用户编号"=?':'');
  const args=[hash,purpose,now,...(purpose==='verify'?[userId]:[])];
