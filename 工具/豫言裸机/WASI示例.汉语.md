...
`工具/豫言裸机/WASI示例/` 是一棵可以直接放进磁盘的目录树：`程序/` 下是二十一个用 wasi-sdk 编成的 `.wasm`（C 程序按 WASI preview1 约定导入 `wasi_snapshot_preview1`、导出 `_start`），`源码/` 下是它们的 C 源文件。壳测试 `WASI` 与 `WASI文件` 用符号链接把它当作预置树，所以每个程序都在两个架构的 QEMU 里逐字验证过输出与退出码。执行器实现了 WASI 的哪一部分，见 [客体二号](客体二号.汉语.md) 的“WASI preview1 子集”一节。
