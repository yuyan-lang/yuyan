...
`工具/豫言裸机/WASI示例/` 是一棵可以直接放进磁盘的目录树：`程序/` 下是十五个用 wasi-sdk 编成的 `.wasm`（C 程序按 WASI preview1 约定导入 `wasi_snapshot_preview1`、导出 `_start`），`源码/` 下是它们的 C 源文件。壳测试 `WASI` 与 `WASI文件` 用符号链接把它当作预置树，所以每个程序都在两个架构的 QEMU 里逐字验证过输出与退出码。执行器实现了 WASI 的哪一部分，见 [客体二号](客体二号.汉语.md) 的“WASI preview1 子集”一节。
...
管道与重定向也能用：`运行 /程序/猫.wasm /数据/文本.txt | /程序/大写.wasm | /程序/字数.wasm`、`运行 /程序/字数.wasm < /数据/文本.txt`、`设 问候 你好` 后 `运行 /程序/环境.wasm 问候`（`/数据/文本.txt` 是这棵树里的一个小文本，由壳测试 `管道` 使用）。
