...
`工具/豫言裸机/WASI示例/` 乃一目录之树，可径置于磁盘：`程序/` 下二十一 `.wasm`，皆 wasi-sdk 所编（C 程序依 WASI preview1 之约，导入 `wasi_snapshot_preview1`，导出 `_start`），`源码/` 下其 C 源文件也。壳测试 `WASI` 与 `WASI文件` 以符号链接令此树为预置之树，故每程序于二架构之 QEMU 中，出与退出码皆逐字验之。执行器所作 WASI 之何部，见 [客体二号](客体二号.文言.md) 之“WASI preview1 之子集”一节。
