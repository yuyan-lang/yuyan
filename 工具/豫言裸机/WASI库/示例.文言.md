...
`工具/豫言裸机/WASI示例/` 乃一目录之树，可径置于磁盘：`程序/` 下二十二 `.wasm`，皆 wasi-sdk 所编（C 程序依 WASI preview1 之约，导入 `wasi_snapshot_preview1`，导出 `_start`），`源码/` 下其 C 源文件也。壳测试 `WASI` 与 `WASI文件` 以符号链接令此树为预置之树，故每程序于二架构之 QEMU 中，出与退出码皆逐字验之。执行器所作 WASI 之何部，见 [客体二号](客体二号.文言.md) 之“WASI preview1 之子集”一节。
...
管与重定向亦可用：`运行 /程序/猫.wasm /数据/文本.txt | /程序/大写.wasm | /程序/字数.wasm`、`运行 /程序/字数.wasm < /数据/文本.txt`、`设 问候 你好` 后 `运行 /程序/环境.wasm 问候`；`/数据/文本.txt`、`/数据/单词.txt` 乃此树中二小文本，壳测试 `管道` 用之，如 `运行 /程序/排序.wasm < /数据/单词.txt | /程序/去重.wasm -c` 以数词频、`运行 /程序/猫.wasm /数据/单词.txt | /程序/找.wasm an | /程序/排序.wasm -r` 先滤后倒序。诸段相继而行（内核唯二任务槽），前段之出存为临时文件而授于后段。
...
| `头 [-n N] [文件…]` | 印每文件（无文件则标准入）之首 N 行（默认十），多文件则每前加 `==> 文件 <==`（head） | 文件与标准入两用、`fgets` 逐行而读 |
| `尾 [-n N] [文件…]` | 印末 N 行（默认十）（tail） | 尽读入而自后数之，`malloc`、`realloc` 倍增 |
| `排序 [-r] [-n] [文件…]` | 依字节序排诸行，`-r` 倒序，`-n` 依行首之整数较之（sort） | `qsort` 与 `strcmp`、标准入尽读 |
| `找 [-v] [-n] [-c] 词 [文件…]` | 印含字面关键词之行，`-v` 反选、`-n` 加行号、`-c` 唯数行数，多文件则行首加 `文件:`（grep -F）；有合者退出码零，无则一 | `strstr`、退出码归于壳 |
| `去重 [-c] [文件]` | 相邻之同行并为一，`-c` 于行首加其现之次（uniq） | 常与 `排序` 连用：`排序 \| 去重 -c` |
| `倒行 [文件]` | 诸行倒序而出（tac） | 管之中环 |
| `日历` | 印 `gmtime(time(NULL))` 之年月日、时与星期，验实时钟睡三十毫秒后进、单调钟亦进、`time()` 与 `clock_gettime` 之秒数相合 | 实时钟（`clock_time_get(realtime)` 出于内核墙钟务，UTC）；壳测试中 QEMU 以 `-rtc base=2031-07-15T08:09:10` 定其始，故出确定 |
