...
豫言值 豫言_传输控制协议_读取字节串(豫言值 套接字参数, 豫言值 最大字节数参数) {
    int 套接字 = (int)豫言值转整数(套接字参数);
    int64_t 最大字节数 = 豫言值转整数(最大字节数参数);
    if (最大字节数 < 0 || 最大字节数 > UINT32_MAX) {
        return 构造传输控制协议结果(传输控制协议错误状态(EINVAL), 分配豫言_字节串缓冲区(0));
    }
    if (最大字节数 == 0) {
        return 构造传输控制协议结果(传输控制协议状态成功, 分配豫言_字节串缓冲区(0));
    }

    unsigned char *缓冲区 = malloc((size_t)最大字节数);
    if (缓冲区 == NULL) {
        return 构造传输控制协议结果(传输控制协议错误状态(ENOMEM), 分配豫言_字节串缓冲区(0));
    }
    ...
    if (读取字节数 > 0) {
        豫言值 内容 = 复制字节为豫言值((uint64_t)读取字节数, 缓冲区);
        free(缓冲区);
        return 构造传输控制协议结果(传输控制协议状态成功, 内容);
    }
    ...
    int 保存错误码 = errno;
    free(缓冲区);
    豫言值 空内容 = 分配豫言_字节串缓冲区(0);
    if (读取字节数 == 0) {
        return 构造传输控制协议结果(传输控制协议状态已关闭, 空内容);
    }
    if (是等待错误(保存错误码)) {
        return 构造传输控制协议结果(传输控制协议状态等待, 空内容);
    }
    return 构造传输控制协议结果(传输控制协议错误状态(保存错误码), 空内容);
}
...
豫言值 豫言_传输控制协议_从字节序数写入字节串(
    ...
    int 套接字 = (int)豫言值转整数(套接字参数);
    const unsigned char *内容 = 豫言值转字节串指针(内容参数);
    uint64_t 内容长度 = 获取豫言_字节串长度(内容参数);
    int64_t 起始字节序数 = 豫言值转整数(起始字节序数参数);
    if (起始字节序数 < 0 || (uint64_t)起始字节序数 > 内容长度) {
        return 构造传输控制协议整数结果(传输控制协议错误状态(EINVAL), 0);
    }
