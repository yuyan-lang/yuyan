...
let get_operators (input : CS.t_string) (result : A.t) : binary_op list =
  ...
  let component_metas : binary_op_meta list =
    List.mapi
      (fun i x ->
      ...
      components
  in
  let final_components : binary_op list =
    List.mapi
      (fun i x ->
         { meta = List.nth component_metas i
         ; reduction =
             (if i <> List.length component_metas - 1
              then p_internal_error ("UDO105: No reduction for non-terminal operator " ^ CS.get_t_string x)
              else
                let* operands, oper_ext =
                  match global_rightfix with
                ...
                in
                if not (List.length operands = List.length parameter_names)
                then
                  failwith ("UDO119: Number of operands does not match number of parameter names " ^ CS.get_t_string x);
