...
let update_binding_term (name : Ext.t_str) (tm : A.t) : unit proc_state_m =
  ...
  let rec update_first = function
    | [] -> []
    | (n, tp, _) :: rest when n = name -> (n, tp, Some tm) :: rest
    | binding :: rest -> binding :: update_first rest
  in
  let new_env = update_first s.env in
  write_proc_state { s with env = new_env }
;;
...
let lookup_binding (name : Ext.t_str) : (A.t * A.t option) proc_state_m =
  ...
  | None -> pfail ("Binding not found: " ^ Ext.get_str_content name)
;;

(* Check if a binding exists *)
let binding_exists (name : Ext.t_str) : bool proc_state_m =
  let* s = get_proc_state () in
  return (List.exists (fun (n, _, _) -> n = name) s.env)
;;
...
let update_constant_term (uid : int) (tm : A.t) : unit proc_state_m =
  ...
  let rec update = function
    ...
    | (id, const) :: rest when id = uid ->
      (match const with
       | Expression { tp; tm = _ } -> Some ((id, Expression { tp; tm = Some tm }) :: rest)
       | _ -> None)
    | binding :: rest ->
