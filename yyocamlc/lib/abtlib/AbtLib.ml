...
module Extent : EXTENT = struct
  ...
  let show_extent_1_based (s : t_extent) : string =
    let file, (row1, col1), (row2, col2) = s in
    if row1 = row2
    then Printf.sprintf "%s:%d:%d-%d" file (row1 + 1) (col1 + 1) (col2 + 1)
    else Printf.sprintf "%s:%d:%d - %d:%d" file (row1 + 1) (col1 + 1) (row2 + 1) (col2 + 1)
  ;;
...
module Abt (NodeClass : NODE_CLASS) : ABT with type node_t = NodeClass.t and type t_extent = Extent.t = struct
  ...
  *)
    ...
    | Subst (b, s) ->
      (match b, s with
       ...
       | BoundVar i, Cons (hd, s) ->
         if i = 1 then subst_head_reduce hd else subst_head_reduce (Subst (BoundVar (i - 1), s))
         ...
         N
           ...
           , List.map
               (fun arg ->
                  ...
                  Subst (arg, s))
               args )
       | Binding (name, under), s -> Binding (name, Subst (under, Cons (BoundVar 1, Comp (s, Upshift))))
  ...
  let rec hereditary_subst_head_reduce (base_t : base_t) : base_t =
    match base_t with
    | Subst (_, _) -> hereditary_subst_head_reduce (subst_head_reduce base_t)
    | BoundVar _ -> base_t
  ...
  let unbind_abt_list (tm : t) (bnd_count : int) : Extent.t_str list * t =
    let rec unbind_rec (sofar : Extent.t_str list) (tm : t) (bnd_count : int) : Extent.t_str list * t =
      ...
      then sofar, tm
        ...
        let name, under_abt = unbind_abt tm in
        unbind_rec (sofar @ [ name ]) under_abt (bnd_count - 1))
    in
  ...
  let appears_in_expr (name : string) (tm : t) : bool = List.mem name (get_free_vars tm)
        ...
        (fun (arg_ctx, arg) ->
           let subst =
             List.fold_right
               ...
               arg_ctx
               (iterative_upshifts (List.length arg_ctx))
           in
           (* actually upshifts here are unnecessary because context enclose arg, no extra free args in arg except those in the context, so we never go above *)
    ...
    | Some outer_name_idx_in_list ->
      ...
      let subst =
        List.fold_right
          ...
          (Cons (BoundVar 1, iterative_upshifts outer_name_idx))
      in
    ...
    let result =
      ...
      | N (node_type, args) ->
        ...
        (match arity with
         ...
         | Some arity_l ->
           if
             List.length arity_l = List.length args
             && List.for_all2 (fun (bnds, _) arity -> List.length bnds = arity) args arity_l
           then fold_no_arity_check view
           else (
             print_endline
               ("fold: arity mismatch for " ^ NodeClass.show node_type ^ " got " ^ show_raw (fold_no_arity_check view));
  ...
  let fold_with_extent (tv : t_view) (extent : t_extent) : t = annotate_with_extent (fold tv) extent
